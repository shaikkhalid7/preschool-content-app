...
# Custom CSS injection for Pastel Montessori Brand Vibe + CSS Keyframe Animations
st.markdown("""
    <style>
    ...
    /* Smooth Entry Animations on Content Containers */
    .animated-header {
...
st.markdown("Welcome to your interactive agency-grade content management hub. Toggle edit mode to customize live scripts and export directly to Excel.")
st.markdown('</div>', unsafe_allow_html=True)

# --- APPLICATION STATE SEED ENGINE ---
# We use st.session_state so that user edits persist smoothly between tab changes
if "content_db" not in st.session_state:
    ...
    st.session_state.content_db = raw_data

# --- SIDEBAR CONTROLS ---
st.sidebar.header("⚙️ Workspace Controls")
edit_mode = st.sidebar.toggle("🔧 Enter Edit Mode", value=False, help="Enable this to dynamically edit captions, scripts, titles, and tags directly inside the app panels.")

st.sidebar.markdown("---")
st.sidebar.header("🎯 Filter Options")
format_filter = st.sidebar.multiselect("Content Formats:", options=["Reel", "Carousel", "Static Image"], default=["Reel", "Carousel", "Static Image"])
search_query = st.sidebar.text_input("Search Content Pillars:")

# --- PROCESSING EXCEL PIPELINE ---
def generate_excel_bytes():
    output = io.BytesIO()
    export_df = pd.DataFrame(st.session_state.content_db)
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        export_df.to_excel(writer, index=False, sheet_name='Master Calendar View')
        for week_name in ["Week 1", "Week 2", "Week 3", "Week 4"]:
            week_df = export_df[export_df['Week'] == week_name]
...
st.sidebar.download_button(
    label="Download Updated Excel (.xlsx)",
    data=generate_excel_bytes(),
    file_name="Preschool_Social_Media_Kit_Live.xlsx",
...
# Filtering Engine
full_master_df = pd.DataFrame(st.session_state.content_db)
filtered_df = full_master_df[full_master_df["Format"].isin(format_filter)]
if search_query:
    filtered_df = filtered_df[filtered_df["Pillar"].str.contains(search_query, case=False) | filtered_df["Title"].str.contains(search_query, case=False)]

with tab1:
    st.subheader("Complete Production View Matrix")
    st.dataframe(filtered_df, use_container_width=True, hide_index=True)

def render_interactive_week(week_label):
    # Loop over the session state directly so edits modify the underlying master arrays live
    found_any = False
    for i, item in enumerate(st.session_state.content_db):
        if item["Week"] != week_label:
            continue
            
        # Apply filters
        if item["Format"] not in format_filter:
            continue
        if search_query and (search_query.lower() not in item["Pillar"].lower() and search_query.lower() not in item["Title"].lower()):
            continue
            
        found_any = True
        
        # HTML wrapper applying custom entry keyframes
        st.markdown(f'<div class="animated-card">', unsafe_allow_html=True)
        
        if not edit_mode:
            # --- VIEW MODE LAYOUT ---
            st.markdown(f"#### 📍 Day {item['Day']} | {item['Format']} — **{item['Title']}**")
            c1, c2 = st.columns([1, 2])
            with c1:
                st.markdown(f"**Strategy Pillar:** `{item['Pillar']}`")
                st.markdown("**🎬 Production Script Layout:**")
                st.caption(item['Visual_Layout_Script'].replace("\n", "  \n"))
                st.info(f"💡 **Call to Action Target:**\n\n{item['CTA']}")
            with c2:
                st.markdown("**📝 Copy Deck:**")
                st.code(item['Caption'], language="text")
                st.caption(f"🏷️ **Hashtags:** {item['Hashtags']}")
        else:
            # --- LIVE EDIT MODE LAYOUT ---
            st.markdown(f"#### 🔧 Editing Panel: Day {item['Day']} ({item['Format']})")
            col_title, col_pillar = st.columns(2)
            with col_title:
                st.session_state.content_db[i]["Title"] = st.text_input(f"Title (Day {item['Day']})", value=item["Title"], key=f"t_{i}")
            with col_pillar:
                st.session_state.content_db[i]["Pillar"] = st.text_input(f"Pillar (Day {item['Day']})", value=item["Pillar"], key=f"p_{i}")
                
            col_left, col_right = st.columns([1, 2])
            with col_left:
                st.session_state.content_db[i]["Visual_Layout_Script"] = st.text_area(f"Production Layout Script", value=item["Visual_Layout_Script"], key=f"v_{i}", height=150)
                st.session_state.content_db[i]["CTA"] = st.text_area(f"Call to Action Metric Target", value=item["CTA"], key=f"c_{i}", height=100)
            with col_right:
                st.session_state.content_db[i]["Caption"] = st.text_area(f"Primary Copy Deck (Paragraphs)", value=item["Caption"], key=f"cap_{i}", height=220)
                st.session_state.content_db[i]["Hashtags"] = st.text_input(f"Hashtag Library System", value=item["Hashtags"], key=f"h_{i}")
                
        st.markdown('</div>', unsafe_allow_html=True)
        
    if not found_any:
        st.info("No records matching active search filters inside this weekly tier block.")

with tab2: st.subheader("Theme: Welcome to Learning"); render_interactive_week("Week 1")
with tab3: st.subheader("Theme: Play is Learning"); render_interactive_week("Week 2")
with tab4: st.subheader("Theme: Growing Together"); render_interactive_week("Week 3")
with tab5: st.subheader("Theme: Ready for Admissions"); render_interactive_week("Week 4")